...
class MuZeroNetwork:
    def __new__(cls, config):
        ...
        elif config.network == "ensemblefullyconnected":
            return EnsembleMuZeroFullyConnectedNetwork(
                config.observation_shape,
                config.stacked_observations,
                len(config.action_space),
                config.encoding_size,
...
##################################
######## Ensemble Fully Connected #########


class EnsembleMuZeroFullyConnectedNetwork(AbstractNetwork):
    def __init__(
        self,
        observation_shape,
        stacked_observations,
        ...
        self.dynamics_encoded_state_network = self._build_ensemble_dynamics_model(encoding_size, fc_dynamics_layers)
        self.dynamics_reward_network = torch.nn.DataParallel(
            mlp(encoding_size, fc_reward_layers, self.full_support_size)
        )
    ...
    def dynamics(self, encoded_state, action, selected_ensemble_model_id=None):
        # Stack encoded_state with a game specific one hot encoded action (See paper appendix Network Architecture)
        action_one_hot = (
            torch.zeros((action.shape[0], self.action_space_size))
            .to(action.device)
        ...
        next_encoded_state, uncertainty = self.dynamics_encoded_state_network(x, selected_id=selected_ensemble_model_id)
    ...
    def recurrent_inference(self, encoded_state, action, selected_ensemble_model_id=None):
        next_encoded_state, reward, uncertainty = self.dynamics(encoded_state, action, selected_ensemble_model_id)
        policy_logits, value = self.prediction(next_encoded_state)
        return value, reward, policy_logits, next_encoded_state, uncertainty

    def _build_ensemble_dynamics_model(self, encoding_size, fc_dynamics_layers):
        models = []
